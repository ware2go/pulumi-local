...
First, clone this repo with the name of the desired provider in place of `local`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-local
```

Second, replace references to `local` with the name of your provider:
...
To use from Go, use `go get` to grab the latest version of the library
    ...
    $ go get github.com/ware2go/pulumi-local/sdk/go/...

## Configuration

The following configuration points are available for the `local` provider:

- `local:apiKey` (environment: `XYZ_API_KEY`) - the API key for `local`
- `local:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
